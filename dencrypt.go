...
var (
	ErrCiphertextBlockSizeTooShort = errors.New("ciphertext block size is too short")
)
...
// EncryptFilename encrypts a file name by given key.
func EncryptFilename(file string, key []byte) (string, error) {
	...
	encodedEncryptedFilename := base62.EncodeToString(append(ciphertext[:4], ciphertext[aes.BlockSize:]...))

	return filepath.Join(path, encodedEncryptedFilename) + ext, nil
}

// DecryptFilename decrypts a file name by given key.
func DecryptFilename(file string, key []byte) (string, error) {
	path := strings.TrimSuffix(file, filepath.Base(file))
	name := strings.Split(filepath.Base(file), ".")[0]

	decodedFilename, err := base62.DecodeString(name)
	if err != nil {
		return "", err
	}
	...
	if len(ciphertext) < aes.BlockSize {
		return "", ErrCiphertextBlockSizeTooShort
	}
...
// NewFilename adds the '.enc' extension to a filename
...
// RestoreFilename removes the '.enc' extension from a filename
func RestoreFilename(filename string) (string, error) {
	base := filepath.Base(filename)
	path := strings.TrimSuffix(filename, base)
